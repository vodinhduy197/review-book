...
<td class="style-td testRomeve{{ $id }}"">
    <a class="text-infor mr-1">
        @if($status == config('define.active'))
            <i class="fas fa-user-lock" onClick="return ajaxActiveStatus({{ $id }}, 0, '{{ route("updateStatus") }}')"></i>
        @else
            <i class="fas fa-unlock-alt" onClick="return ajaxActiveStatus({{ $id }}, 1, '{{ route("updateStatus") }}')"></i>
        @endif
    </a>
